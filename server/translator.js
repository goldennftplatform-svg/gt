  ...
  if (!previous) {
    return [
      event({
        ...
        summary: "Captured a baseline of the app, network, models, and tools. Next sniffs will call out what changed in plain English.",
        details: {
  ...
  if (prevBuild && currBuild && prevBuild !== currBuild) {
    events.push(
      event({
        ...
        severity: "high",
        heat: heatFor("deploy"),
        title: "Geoff website shipped an update",
        summary: `A new app build went live (${short(prevBuild)} → ${short(currBuild)}). Users may see new UI or behavior.`,
  ...
  if (prevDeploy && currDeploy && prevDeploy !== currDeploy) {
    events.push(
      event({
        ...
        severity: "high",
        heat: heatFor("deploy"),
        title: "New web deploy published",
        summary: `Hosting rolled a fresh deploy (${prevDeploy} → ${currDeploy}). Usually means the live site just updated.`,
  ...
  if (prevChunk && currChunk && prevChunk !== currChunk) {
    events.push(
      event({
        ...
        title: "Site assets refreshed",
        summary: `JavaScript bundles changed (${prevChunk} → ${currChunk}). Often accompanies a UI tweak or feature drop.`,
        details: {
          from: prevChunk,
          to: currChunk,
  ...
  if (prevVer && currVer && prevVer !== currVer) {
    events.push(
      event({
        ...
        title: "AI network software upgraded",
        summary: `Stacknet moved ${prevVer} → ${currVer}. Under-the-hood runtime changes for generation and agents.`,
        details: {
          from: prevVer,
          to: currVer,
  ...
  if (prevHealth && currHealth && prevHealth !== currHealth) {
    events.push(
      event({
        ...
        heat: heatFor("health"),
        title: "Network health flipped",
        summary: `Status went ${prevHealth} → ${currHealth}. If unhealthy, generations may fail or slow down.`,
  ...
  if (isNumber(nodesPrev) && isNumber(nodesCurr) && nodesPrev !== nodesCurr) {
    events.push(
      event({
        ...
        summary: `Live nodes ${nodesPrev} → ${nodesCurr}. More nodes usually means more capacity.`,
        details: {
          from: nodesPrev,
          to: nodesCurr,
  ...
  if (isNumber(gpusPrev) && isNumber(gpusCurr) && gpusPrev !== gpusCurr) {
    events.push(
      event({
        ...
        summary: `GPUs available ${gpusPrev} → ${gpusCurr}. Affects how much image/video/music work the network can handle.`,
    ...
    if (capShifts.length) {
      events.push(
        event({
          ...
          title: "A model learned new tricks",
          summary: capShifts
            .slice(0, 3)
  ...
  if (mcpPrev && mcpCurr && mcpPrev !== mcpCurr) {
    events.push(
      event({
        ...
        severity: "high",
        heat: heatFor("version"),
        title: "Agent plug-in contract updated",
        summary: `Outside AI agents may need the new MCP contract (${short(mcpPrev)} → ${short(mcpCurr)}).`,
